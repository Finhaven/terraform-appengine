...
variable "gae_environment" {
  type        = string
  default     = "standard"
  description = "The GAE environment to use for the service. Must be 'standard' or 'flexible'."
  nullable    = false

  validation {
    condition     = contains(["standard", "flexible"], var.gae_environment)
    error_message = "Valid values for var: gae_environment are (standard and flexible)."
  }
}
...
variable "health_check_paths" {
  description = "Liveness and readiness check endpoints for GAE Flexible environment instances."
  type = object({
