...
locals {
  version_hashes = jsondecode(module.state.stdout)
  ...
  allocations = {
    for i, hash in local.version_hashes :
    hash => i == 0 ? 1 : 0
  }

  # Build the version specific settings for the GAE service versions.
  versions = {
    for hash in local.version_hashes :
    hash => {
      service_version = hash
      zip = {
...
output "data" {
  value = {
    hashes   = local.version_hashes
    current  = try(local.version_hashes[0], null)
